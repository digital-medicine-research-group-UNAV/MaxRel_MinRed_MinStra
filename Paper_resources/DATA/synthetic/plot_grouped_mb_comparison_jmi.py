...
Usage:
  pixi run python DATA/synthetic/plot_grouped_mb_comparison_jmi.py
  pixi run python DATA/synthetic/plot_grouped_mb_comparison_jmi.py --results_root /path/to/RESULTS/synthetic
  pixi run python DATA/synthetic/plot_grouped_mb_comparison_jmi.py --output_dir /path/to/output
...
def _compute_pairwise_statistics(
    ...
    # Compare JMI-MS only against the other methods.
...
def _print_comparison_report(observation_scores: pd.DataFrame, pairwise_stats: pd.DataFrame) -> None:
    ...
    smfs_250 = get_stats("Smfs", "JMI-MS", 250)
    smfs_1000 = get_stats("Smfs", "JMI-MS", 1000)
    smfs_5000 = get_stats("Smfs", "JMI-MS", 5000)
    jmi_250 = get_stats("JMI", "JMI-MS", 250)
    jmi_1000 = get_stats("JMI", "JMI-MS", 1000)
    jmi_5000 = get_stats("JMI", "JMI-MS", 5000)

    caption = (
        "Distribution of grouped Markov blanket recall across synthetic experiments for studied "
        "sample sizes, comparing {\\tt Smfs} (red), {\\tt JMI} (blue), and {\\tt JMI-MS} "
        ...
        "involving JMI-MS, assessed with two-sided paired Wilcoxon signed-rank tests and adjusted "
        "across sample sizes for each pairwise comparison using the Holm-Bonferroni procedure. "
        f"For Smfs vs. JMI-MS, the raw and adjusted p-values were ${_format_caption_p_value(smfs_250['p_raw'])}$ "
        ...
        f"${smfs_5000['cohens_dz']:.3f}$, respectively. For JMI vs. JMI-MS, the corresponding "
        f"raw and adjusted p-values were ${_format_caption_p_value(jmi_250['p_raw'])}$ and "
        f"${_format_caption_p_value(jmi_250['p_adj'])}$ at $n = 250$, "
        f"${_format_caption_p_value(jmi_1000['p_raw'])}$ and ${_format_caption_p_value(jmi_1000['p_adj'])}$ "
        f"at $n = 1000$, and ${_format_caption_p_value(jmi_5000['p_raw'])}$ and "
        f"${_format_caption_p_value(jmi_5000['p_adj'])}$ at $n = 5000$, with paired Cohen’s $d_z$ "
        f"values of ${jmi_250['cohens_dz']:.3f}$, ${jmi_1000['cohens_dz']:.3f}$, and "
        f"${jmi_5000['cohens_dz']:.3f}$, respectively."
    )
    print(caption)
...
def plot_grouped_boxplot(df: pd.DataFrame, out_dir: Path) -> list[Path]:
    ...
    out_pdf = out_dir / "boxplot_recall_grouped_smfs_jmi_jmims.pdf"
    out_svg = out_dir / "boxplot_recall_grouped_smfs_jmi_jmims.svg"
    fig.savefig(out_pdf)
    fig.savefig(out_svg)
    plt.close(fig)
