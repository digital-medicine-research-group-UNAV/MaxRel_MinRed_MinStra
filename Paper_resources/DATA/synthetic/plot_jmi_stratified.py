...
Usage:
  pixi run python DATA/synthetic/plot_jmi_stratified.py
  pixi run python DATA/synthetic/plot_jmi_stratified.py --mb_threshold 5
  pixi run python DATA/synthetic/plot_jmi_stratified.py --results_root /path/to/RESULTS/synthetic
  pixi run python DATA/synthetic/plot_jmi_stratified.py --output_dir /path/to/output
...
def _print_comparison_report(
    ...
    print(f"[{stratum_label}] Pairwise Wilcoxon summary (JMI-MS vs baselines):")
...
def plot_grouped_boxplot_stratified(df: pd.DataFrame, out_dir: Path, mb_threshold: int) -> list[Path]:
    ...
    out_pdf = out_dir / f"boxplot_recall_grouped_smfs_jmi_jmims_stratified_mbthr_{mb_threshold}.pdf"
    out_svg = out_dir / f"boxplot_recall_grouped_smfs_jmi_jmims_stratified_mbthr_{mb_threshold}.svg"
    fig.savefig(out_pdf)
    fig.savefig(out_svg)
    plt.close(fig)
